                        ...
                        if (j != k) {
                            if (Vehicle.getTo().getName().equals(wantedCities.get(k - 1).getName())) {
                                if (isMoney) graph[j][k] = Vehicle.getPrice();
                                    ...
                                    graph[j][k] = Vehicle.getTimeInWay();//getTimeInWay(Vehicle.getDateTo(), Vehicle.getDateFrom());
                                }
                                else
                                    graph[j][k] = m * Vehicle.getPrice() + t *Vehicle.getTimeInWay();// getTimeInWay(Vehicle.getDateTo(), Vehicle.getDateFrom());
                            }
                        }
                    }
                }
        ...
        for (double[] aGraph : graph) {
            for (int p = 0; p < graph.length; p++) {
                if (aGraph[p] > 100000000) {
                    ...
                    str = String.valueOf(Double.valueOf(aGraph[p]).doubleValue());
                }
