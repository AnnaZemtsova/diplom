...
function controlButton() {
    if(this.lastChild.nodeValue == '<'||this.lastChild.nodeValue == '<<'){
    	if(date.getMonth()==0){
	...
	/*if (this.lastChild.nodeValue.length == 1) {
		...
		var m = (this.lastChild.nodeValue == '<')? date.getTime() - (24 * 60 * 60 * 1000 * date.getDate()) : date.getTime()
			+ (24 * 60 * 60 * 1000 * (getCountDay(date.getMonth()) - date.getDate() + 1));
			date.setTime(m);
	}
	else {
        /*var m = (this.lastChild.nodeValue == '>')? date.getTime() - (24 * 60 * 60 * 1000 * date.getDate()) : date.getTime()
		...
		for (j = 0; j < 7; j++) {
			...
			if (flag && countday <= getCountDay(date.getMonth())) {
				...
				cell.onclick = function() {
                    var dd = this.lastChild.nodeValue;
                    var dc = currdate.getDate();
                    var md = date.getMonth();
                    var mc = currdate.getMonth();
                    var yd = date.getFullYear();
                    var yc = currdate.getFullYear();
                        document.getElementById('calendar').style.display = 'none';
                        date.setDate(this.lastChild.nodeValue);
                        var d = (date.getDate() < 10) ? '0' + date.getDate() : date.getDate();
                        var m = ((date.getMonth() + 1) < 10) ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1);
                    if(((dd<dc&&md<=mc&&yd<=yc)||
                        (md<mc&&yd<=yc)||
                        (yd<yc))){
                    }
                    else {
                    	input.value =date.getFullYear()+'-'+m+'-'+d;//d + '.' + m + '.' + date.getFullYear();
                    }
